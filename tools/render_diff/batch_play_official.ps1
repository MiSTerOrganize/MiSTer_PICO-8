...
$wrap  = "C:\Users\miste\AppData\Local\Temp\rd_run\play_wrap"
$outd  = "C:\Users\miste\AppData\Local\Temp\rd_run\play_off"
...
foreach ($f in $files) {
  ...
  $n++; if ($n % 100 -eq 0) { Write-Host "official-play $n / $tot" }
}
Write-Host "official-play complete: $((Get-ChildItem "$outd\*.txt").Count)"
